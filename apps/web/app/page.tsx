...
function BrandMark({ compact = false }: { compact?: boolean }) {
  return (
    <span className="inline-flex items-center gap-2.5" aria-label="Proof of Work">
      <span className="brand-mark" aria-hidden="true">
        <GitBranch className="size-4" strokeWidth={2.4} />
      </span>
      {!compact ? <span className="text-[15px] font-semibold tracking-[-0.02em]">Proof of Work</span> : null}
    </span>
  );
}
